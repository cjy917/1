...
const iframeSrc = computed(() => {
  ...
  if (props.autoplay && props.source.type === 'youtube') {
    ...
    if (!url.includes('autoplay=1')) {
      url = `${url}${join}autoplay=1&mute=1&controls=${ctrl}&rel=0&modestbranding=1&playsinline=1${heroExtras}`
    } else if (props.heroMode && !url.includes('iv_load_policy=3')) {
      url = `${url}${heroExtras}`
    }
  }
  if (props.autoplay && props.source.type === 'bilibili') {
    const join = url.includes('?') ? '&' : '?'
    if (!url.includes('autoplay=1')) {
      url = `${url}${join}autoplay=1&muted=1`
    }
  }
  return url
})
...
function onVideoCanPlay() {
  if (isMp4.value && (props.heroMode || props.cardMode)) {
    confirmPlaying()
  }
}
...
watch(
  () => [props.autoplay, props.source?.embed_url, props.source?.type, mp4Src.value],
  async () => {
    resetPlaybackWatch()
    if (!props.autoplay || !isMp4.value || !videoRef.value) return
    try {
