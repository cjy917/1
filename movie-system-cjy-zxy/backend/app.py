...
from services.comment_service import get_home_section_comments
...
from services.streaming_service import resolve_watch_links
...
def create_app() -> Flask:
    ...
    def register():
        """【A1】POST /api/auth/register — AuthView.handleRegister"""
    ...
    def login():
        """【A1】POST /api/auth/login — AuthView.handleLogin"""
        payload = request.get_json(force=True)
        username = (payload.get("username") or "").strip()
        password = payload.get("password") or ""
    ...
    def me():
        """【A3】GET /api/auth/me — 前端路由守卫与 userStore.fetchMe"""
    ...
    # =========================================================================
    # 【首页 + 详情页 · 电影 API】在 app.py 搜路由路径即可
    # 前端连接: frontend/src/api/index.js → movieApi / commentApi 等
    # 速查索引: frontend/src/code-map.js
    # =========================================================================

    @app.route("/api/movies/home")
    def home_sections():
        """【首页-H1】GET /api/movies/home → movie_service.get_home_sections"""
        return jsonify(get_home_sections())

    @app.route("/api/movies/home/comments")
    def home_section_comments():
        """【首页-H4】GET /api/movies/home/comments → comment_service.get_home_section_comments"""
        limit = min(int(request.args.get("limit", 30)), 50)
        return jsonify(get_home_section_comments(limit_per_section=limit))

    @app.route("/api/movies/filters")
    def movie_filters():
        return jsonify(get_filter_options())

    # ========== 偏好问卷：导演 / 演员选项 · AuthView A2 ==========
    @app.route("/api/preferences/directors")
    def preference_directors():
        """【A2】GET /api/preferences/directors — 问卷导演气泡"""
        ...
        with get_mysql() as conn:
            with conn.cursor() as cursor:
                ...
                for row in cursor.fetchall():
                    for director in split_pipe(row["directors"]):
                        if director and director != "未知":
                            counter[director] += 1

        return jsonify({"items": [{"name": name, "count": count} for name, count in counter.most_common(30)]})

    @app.route("/api/preferences/actors")
    def preference_actors():
        """【A2】GET /api/preferences/actors — 问卷演员气泡"""
        from collections import Counter
    ...
    def movie_detail(movie_id: int):
        """【详情-D0】GET /api/movies/<id> → movie_service.get_movie_by_id"""
        ...
        schedule_summary_enrich(movie_id)
    ...
    def movie_trailer(movie_id: int):
        """【首页-H2 / 详情-D1e】GET /api/movies/<id>/trailer → trailer_service.resolve_trailer"""
        movie = get_movie_by_id(movie_id)
        if not movie:
            return jsonify({"error": "电影不存在"}), 404
    ...
    def movie_playback(movie_id: int):
        """【详情-D1d/D2】GET /api/movies/<id>/play → video_service.resolve_playback"""
        movie = get_movie_by_id(movie_id)
        if not movie:
            return jsonify({"error": "电影不存在"}), 404
    ...
    @app.route("/api/movies/<int:movie_id>/watch-links")
    def movie_watch_links(movie_id: int):
        """【详情-D2a】GET /api/movies/<id>/watch-links → streaming_service.resolve_watch_links"""
        movie = get_movie_by_id(movie_id)
        if not movie:
            return jsonify({"error": "电影不存在"}), 404
        return jsonify(resolve_watch_links(movie))
    ...
    def translate_route():
        ...
        try:
            return jsonify(translate_text(text, target=target))
    ...
    @app.route("/api/recommend/preference", methods=["POST"])
    def preference_recommend():
        """【首页·偏好推荐】根据问卷偏好（类型/导演/演员）返回电影列表"""
        payload = request.get_json(force=True) or {}
        items = get_preference_recommendations(
            payload.get("likeGenres"),
            payload.get("likeDirectors"),
            payload.get("likeActors"),
        )
        return jsonify({"items": items})
    ...
    def graph_similar(movie_id: int):
        """【详情-D4】GET /api/recommend/similar/<id> → recommendation_service.get_content_similar_movies"""
        return jsonify(
            {
