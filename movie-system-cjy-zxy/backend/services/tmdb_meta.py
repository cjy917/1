...
_tmdb_overview_cache: dict[str, str] = {}
...
def _fetch_tmdb_movie(tmdb_id: str, language: str = "zh-CN") -> dict | None:
...
def fetch_tmdb_overview(tmdb_id: str) -> str | None:
    if not tmdb_id:
        return None
    if tmdb_id in _tmdb_overview_cache:
        return _tmdb_overview_cache[tmdb_id]

    for language in ("zh-CN", "zh", "en-US"):
        data = _fetch_tmdb_movie(tmdb_id, language)
        if not data:
            continue
        overview = (data.get("overview") or "").strip()
        if overview:
            _tmdb_overview_cache[tmdb_id] = overview
            return overview
    return None


def fetch_tmdb_backdrop_url(tmdb_id: str) -> str | None:
    data = _fetch_tmdb_movie(tmdb_id, "zh-CN")
    if not data:
        return None
