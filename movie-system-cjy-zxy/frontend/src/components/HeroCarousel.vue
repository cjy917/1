<!--
  【首页·区块1】顶部大轮播 Hero
  代码搜索: loadTrailer / openDetail / toggleTrailerSound
  API: movieApi.trailer → GET /api/movies/:id/trailer → trailer_service.resolve_trailer
  速查索引: src/code-map.js (H2)
-->
<script setup>
import { computed, onBeforeUnmount, onMounted, ref, watch } from 'vue'
import { useRouter } from 'vue-router'
import { movieApi } from '../api'
import { applyLocalTrailerIfExists, isLocalTrailer, isRemoteTrailer, tryLocalTrailerFallback } from '../utils/trailerCache'
...
const trailerPaused = ref(false)
const trailerSoundOn = ref(true)
...
const heroVideoActive = computed(() => showVideo.value && !trailerPaused.value)
...
watch(current, (movie) => {
  ...
  trailer.value = null
  trailerPlaying.value = false
  trailerPaused.value = false
  trailerSoundOn.value = true
...
async function loadTrailer() {
  ...
  try {
    /** 【API·首页轮播】GET /api/movies/:id/trailer */
...
function onHeroClick(event) {
  if (event.target.closest('.hero-arrow, .hero-strip, .hero-trailer-controls, .hero-trailer-control, button, a')) return
  openDetail()
}

function toggleTrailerPause() {
  if (trailerPaused.value) {
    trailerPaused.value = false
    trailerSoundOn.value = true
    return
  }
  trailerPaused.value = true
}

function toggleTrailerSound() {
  trailerSoundOn.value = !trailerSoundOn.value
  if (trailerPaused.value) {
    trailerPaused.value = false
  }
}
...
<template>
  <section
    ...
    :class="{ 'hero--playing': heroVideoActive }"
    @click="onHeroClick"
  >
    <div
      class="hero-backdrop"
      :class="{
        'hero-backdrop--video': heroVideoActive,
    ...
    <div
      ...
      :class="{ 'hero-video-wrap--active': heroVideoActive }"
    >
      <MediaPlayer
        :key="`${current.movie_id}-${trailerSoundOn ? 'sound' : 'mute'}`"
        :source="trailer"
        autoplay
        :muted="!trailerSoundOn"
        :paused="trailerPaused"
        loop
        cover
        hero-mode
        :play-check-ms="8000"
    ...
    <div v-if="shouldTryTrailer && trailerPlaying" class="hero-trailer-controls">
      <button
        type="button"
        class="hero-trailer-control"
        :aria-label="trailerSoundOn ? '静音预告片' : '开启预告片声音'"
        @click.stop="toggleTrailerSound"
      >
        {{ trailerSoundOn ? '🔊' : '🔇' }}
      </button>
      <button
        type="button"
        class="hero-trailer-control"
        :aria-label="trailerPaused ? '播放预告片' : '暂停预告片'"
        @click.stop="toggleTrailerPause"
      >
        {{ trailerPaused ? '▶' : '⏸' }}
      </button>
    </div>
